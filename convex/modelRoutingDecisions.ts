...
async function workflowRunProjectId(
  ...
  workflowRun: { projectId?: Id<"projects">; workOrderId?: Id<"workOrders">; parentTaskId?: Id<"tasks"> },
) {
...
export const listRecent = query({
  ...
  handler: async (ctx, args) => {
    ...
    return ctx.db
      ...
      .order("desc")
      .take(Math.max(1, Math.min(Math.floor(args.limit ?? 50), 200)));
  },
});
...
export const getForTask = query({
  ...
  handler: async (ctx, args) => {
    ...
    await requireWorkspacePermission(ctx, projectId, FACTORY_PERMISSIONS.VIEW);
    if (task.projectId && taskWorkOrder?.projectId && task.projectId !== taskWorkOrder.projectId) {
    ...
    const workOrderDecision = workOrder
      ...
      : null;
    return {
      decision: directDecision?.projectId === projectId
        ? directDecision

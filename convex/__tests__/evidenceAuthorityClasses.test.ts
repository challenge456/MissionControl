...
describe("currentness and scope", () => {
  ...
  it("rejects an otherwise complete attestation with no expiry bound", () => {
    const decision = evaluateCiMergeAuthority({
      row: attested({ attestationExpiresAt: undefined }),
      expectedHeadSha: "cafe1234",
      expectedProviderRepositoryId: "9001",
      now: 1_000,
    });
    expect(decision).toMatchObject({ satisfied: false, refusal: "ATTESTATION_EXPIRED" });
  });
});

...
export interface PrCheckProvenance {
  ...
  /** Server-issued bound after which currentness must be re-observed. */
...
export function evaluateCiMergeAuthority(input: {
  ...
  if (row.providerRepositoryId !== input.expectedProviderRepositoryId) {
    return {
      satisfied: false,
  ...
  if (row.headSha !== input.expectedHeadSha) {
    return {
      satisfied: false,
      refusal: "STALE_HEAD",
      authority: resolution.authority,
  ...
  if (row.attestationExpiresAt === undefined || row.attestationExpiresAt <= input.now) {
    return {
      ...
      authority: resolution.authority,
      detail: row.attestationExpiresAt === undefined
        ? "The external CI attestation has no expiry bound and must be re-observed."
        : "The external CI attestation has expired and must be re-observed.",
    };
  }

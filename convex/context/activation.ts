...
const activatedPackageArg = v.object({
...
/** Activate context for the actual execution run used by Pi/external executors. */
export const activateForWorkflowRun = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const prior = await ctx.db.query("workflowContextActivationReceipts")
    ...
    const lock = await ctx.db.query("contextLocks").withIndex("by_repo", (q) => q.eq("repoSlug", args.repoSlug)).unique();
    if (!lock) throw new Error(`No stored context lock for repository "${args.repoSlug}"`);
    const resolved = parseResolvedContextLock(lock.lockJson);
    const packages: Array<{ packageSlug: string; packageId: any; versionId: any; version: string; contentHash: string; content: string }> = [];
    for (const [packageSlug, entry] of Object.entries(resolved).sort(([a], [b]) => a.localeCompare(b))) {
      const pkg = await ctx.db.query("contextPackages").withIndex("by_slug", (q) => q.eq("slug", packageSlug)).unique();
      if (!pkg) throw new Error(`Locked package "${packageSlug}" no longer exists in the Registry`);
      const version = await ctx.db.query("contextPackageVersions")
        .withIndex("by_package_version", (q) => q.eq("packageId", pkg._id).eq("version", entry.version)).unique();
      if (!version || version.status !== "PUBLISHED") throw new Error(`Locked package "${packageSlug}" version ${entry.version} is not published`);
      if (version.contentHash !== entry.contentHash) throw new Error(`Locked package "${packageSlug}" content hash does not match Registry version ${entry.version}`);
      if (!version.inlineContent) throw new Error(`Locked package "${packageSlug}" version ${entry.version} has no inline executor content`);
      packages.push({ packageSlug, packageId: pkg._id, versionId: version._id, version: version.version, contentHash: version.contentHash, content: version.inlineContent });
    }

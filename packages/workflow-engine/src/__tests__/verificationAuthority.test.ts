...
describe("adversarial candidates cannot manufacture verification success", () => {
  ...
  it("rejects an authority allowance that has no frozen rationale", () => {
    const evaluation = evaluateVerificationAuthority({
      candidate: candidate({ changedFiles: ["vitest.config.ts"] }),
      checks: [TEST_CHECK],
      policy: { allowedSurfaceMutations: ["TEST_CONFIG"] },
    });
    expect(evaluation.status).toBe("FAIL");
    expect(evaluation.findings[0].paths).toEqual(["vitest.config.ts"]);
  });
...
describe("independence requirement levels (D1)", () => {
  ...
  it("fails closed for an executable with no registered authority policy", () => {
    const resolution = resolveCheckIndependence({
      verifierId: "factory-command/v1",
      command: { executable: "new-test-runner", args: ["verify"] },
      mutatedSurfaces: [],
    });
    expect(resolution.independent).toBe(false);
    expect(resolution.reason).toMatch(/no registered definition-authority policy/);
  });

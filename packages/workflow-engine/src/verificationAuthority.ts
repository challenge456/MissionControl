...
export function resolveCommandAuthority(command: {
  ...
  return {
    // Unknown is not trusted. Adding a new executable to the verification
    // allowlist must also classify the repository surfaces that define it;
    // otherwise a new runner silently becomes independent by omission.
    authority: "CANDIDATE_DEFINED",
    definedBy: [],
    reason:
      "The executable has no registered definition-authority policy, so independence cannot be established.",
  };
}
...
export function evaluateVerificationAuthority(input: {
  ...
  const allowanceHasReason = Boolean(input.policy?.reason?.trim());
...
export function resolveCheckIndependence(input: {
  ...
  if (resolution.definedBy.length === 0) {
    return {
      independent: false,

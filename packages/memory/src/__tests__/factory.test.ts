...
describe("Factory Memory", () => {
  it("runs the deterministic five-phase golden path", async () => {
    ...
    expect(result.verificationAdvisory.advisoryOnly).toBe(true);
    expect(result.verificationAdvisory.acceptanceAuthority).toBe(false);
    expect(
      result.verificationAdvisory.checks.every(
        (check) => check.evidenceRequired,
      ),
    ).toBe(true);
    expect(
  ...
  it("redacts source text and structured metadata before indexing", () => {
    const built = buildFactoryMemoryDocument({
      ...
      sourceId: "credential=sensitive-note-token",
      title: "password=hidden-title-token",
      path: "docs/architecture.md?access_token=hidden-path-token",
    ...
    expect(built.document.sourceId).not.toContain("sensitive-note-token");
    expect(built.document.title).not.toContain("hidden-title-token");
    expect(built.document.provenance.path).not.toContain("hidden-path-token");
    expect(built.document.sourceRevision).not.toContain(
  ...
  it("keeps frozen package digests stable and exposes retry diffs", async () => {
    ...
    expect(result.contextPackage.acceptanceAuthority).toBe(false);
    expect(Object.isFrozen(result.contextPackage)).toBe(true);
    expect(Object.isFrozen(result.contextPackage.items)).toBe(true);
  ...
  it("produces a reproducible retrieval plan independent of creation time", () => {
    const fixture = createGoldenPathFixture();
    const first = planContextRetrieval(
      fixture.workOrder,
      { maxItems: 12, maxEstimatedTokens: 12_000 },
      FIXTURE_TIME,
    );
    const second = planContextRetrieval(
      fixture.workOrder,
      { maxItems: 12, maxEstimatedTokens: 12_000 },
      FIXTURE_TIME + 60_000,
    );

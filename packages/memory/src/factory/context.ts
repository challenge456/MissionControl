...
export function assembleContextPackage(input: {
  ...
  for (const item of contextPackage.items) {
    ...
    if (item.relationshipPath) {
      for (const step of item.relationshipPath) Object.freeze(step);
      Object.freeze(item.relationshipPath);
    }
    Object.freeze(item);
  }
  Object.freeze(contextPackage.items);
  Object.freeze(contextPackage.budget);
  return Object.freeze(contextPackage);
}
...
export function buildVerificationAdvisory(input: {
  contextPackage: ContextPackage;
  ...
  return {
    id: stableId(input.contextPackage.id, "verification-advisory", createdAt),

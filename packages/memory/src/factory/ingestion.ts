...
export function buildFactoryMemoryDocument(input: BuildDocumentInput): {
  ...
  const sourceId = redactFactoryMemoryText(input.sourceId, 1_000);
  const title = input.title
    ? redactFactoryMemoryText(input.title, 500)
    : undefined;
  const path = input.path
    ? redactFactoryMemoryText(input.path, 2_000)
    : undefined;
  const revision = input.revision
    ? redactFactoryMemoryText(input.revision, 500)
    : undefined;
  ...
  return {
    ...
    redactionCount:
      ...
      sanitizedMetadata.redactionCount +
      sourceId.redactionCount +
      (title?.redactionCount ?? 0) +
      (path?.redactionCount ?? 0) +
      (revision?.redactionCount ?? 0),
  };
}
...
export function buildCodeGraphProjection(input: {
  ...
  const safePath = redactFactoryMemoryText(path, 2_000).value;
  const safeRevision = redactFactoryMemoryText(revision, 500).value;
  const provenance: MemoryProvenance = {
  ...
  const fileEntity: FactoryEntity = {
    ...
    key: `file:${safePath}`,
    label: safePath,
    aliases: [safePath.split("/").at(-1) ?? safePath],
    metadata: { path: safePath },
    provenance: [provenance],
    createdAt: timestamp,
    updatedAt: timestamp,
  };
  const facts = extractTypeScriptCodeFacts(safePath, content);
  ...
  for (const symbol of facts.symbols) {
    ...
    entities.push({
      ...
      id: symbolId,
      type: "symbol",
      key: `symbol:${safePath}#${symbol}`,
      label: symbol,
      aliases: [],
      metadata: { path: safePath, language: facts.language },
      provenance: [provenance],
      createdAt: timestamp,
      updatedAt: timestamp,
    });
  ...
  for (const importedPath of facts.imports) {
    const safeImportedPath = redactFactoryMemoryText(importedPath, 2_000).value;
    ...
    entities.push({
      ...
      id: moduleId,
      type: "module",
      key: `module:${safeImportedPath}`,
      label: safeImportedPath,
      aliases: [],
      provenance: [provenance],
      createdAt: timestamp,
      updatedAt: timestamp,

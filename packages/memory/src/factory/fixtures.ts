...
export async function runFactoryMemoryGoldenPath() {
  ...
  const verificationAdvisory = buildVerificationAdvisory({
    contextPackage,
  ...
  const evals = evaluateContextPackage({
    ...
    verificationInfluenceCount: verificationAdvisory.checks.filter(
  ...
  const assertions = [
    ...
    [
      "advisory verification context referenced historical risk without acceptance authority",
      verificationAdvisory.checks.some((check) =>

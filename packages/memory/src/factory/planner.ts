...
export function planContextRetrieval(
  ...
  const requiredSourceTypes = [...required].sort();
  const planContentHash = contentHash(
    JSON.stringify({
...
export async function runBoundedRetrievalLoop(input: {
  ...
  const maxCandidates = Math.max(
    20,
    Math.min(400, (input.plan.budget.maxItems ?? 20) * 8),
  );
  const rankedResults = (): FactoryMemoryResult[] =>
    [...results.values()].sort(
      (left, right) =>
  ...
  const boundedResults = (): FactoryMemoryResult[] => {
    const maxItems = Math.max(1, input.plan.budget.maxItems ?? 20);
    const maxTokens = Math.max(
      1,
      input.plan.budget.maxEstimatedTokens ?? Number.MAX_SAFE_INTEGER,
    );
  ...
  for (
    ...
    for (const originalStep of input.plan.steps) {
      ...
      for (const result of stepResults.slice(0, maxCandidates)) {
      ...
      if (results.size > maxCandidates) {
        ...
        for (const chunkId of results.keys())
          if (!retained.has(chunkId)) results.delete(chunkId);
      }
      observations.push({
        type:
    ...
    const selectedResults = boundedResults();

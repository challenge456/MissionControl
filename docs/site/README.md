# Mission Control documentation

Tessl-style documentation for the **agentic software factory** control plane.

## Browse in-app

Open **Knowledge → Docs → Documentation** in the EOS sidebar (`pnpm dev:demo`).

## Site map

### Product direction
- [North Star and V1 strategy](./overview/product-direction.md)

### Operator control plane
- [Decision Center and Operator Evals](./operator-control-plane/overview.md)
- [Integrated V1 validation](../validation/2026-07-31-integrated-control-plane-evidence.md)

### Deterministic Skill Automations
- [Overview](./skill-automations/overview.md)
- [Implementation design](./skill-automations/implementation-design.md)
- [Browser and E2E evidence](./skill-automations/browser-evidence.md)
- [Factory enhancement roadmap](./skill-automations/factory-roadmap.md)
- [Bounded adapter runtime](./skill-automations/bounded-adapter-runtime.md)
- [Automation backend reference](./skill-automations/automation-backend.md)
- [Completed implementation record](./skill-automations/completed-implementation.md)

### Software Factory enhancement
- [Factory Learning and continuous improvement](./software-factory-enhancement/factory-learning.md)
- [Progressive Factory experience](./software-factory-enhancement/progressive-factory-experience.md)

### Overview
- [What is Mission Control?](./overview/readme.md)
- [Platform components](./overview/platform-components.md)

### Get started
- [Set up Mission Control](./get-started/set-up-mission-control.md)
- [Configure workspaces and repositories](./get-started/configure-workspaces-and-repositories.md)
- [Run the demo](./get-started/run-the-demo.md)
- [Improve your first skill](./get-started/improve-your-first-skill.md)

### Tutorials
- [Tutorial index](./tutorials/tutorials.md)
- [Governing WorkOrders](./tutorials/governing-work-orders.md)
- [Setting up agentic code review](./tutorials/setting-up-agentic-code-review.md)
- [Improving a skill](./tutorials/improving-a-skill.md)
- [Protecting against insecure skills](./tutorials/protecting-against-insecure-skills.md)
- [Automating repetitive tasks](./tutorials/automating-repetitive-tasks.md)

### Harness
- [Software factory](./harness/software-factory.md)
- [Architect mode](./harness/architect-mode.md)
- [Code review wizard](./harness/code-review-wizard.md)

### Registry
- [Discover and install](./registry/discover-and-install.md)
- [Context CDL](./registry/context-cdl.md)
- [Eval runs](./registry/eval-runs.md)

### Reference
- [Glossary](./reference/glossary.md)
- [EOS navigation](./reference/eos-navigation.md)
- [Feature flags](./reference/feature-flags.md)

## For AI agents

Full index: [llms.txt](./llms.txt)

## External inspiration

Platform docs structure inspired by [Tessl](https://docs.tessl.io/) — adapted for Mission Control's Convex factory model.

## Legacy repo docs

Deep implementation docs remain in `/docs/` (PRD, BACKEND_STRUCTURE, software-factory/, runbooks). Use the in-app **Search** and **Chat with Repo** tabs to query them semantically.

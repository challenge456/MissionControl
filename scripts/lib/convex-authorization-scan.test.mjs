...
describe("convex authorization scan", () => {
  ...
  it("does not treat comments or strings as authorization calls", () => {
    const root = fixture({
      "spoofed.ts": `
        export const commentOnly = mutation({
          args: {},
          handler: async (ctx) => {
            // requireWorkspaceAccess(ctx, "tenant", "workspace", {});
    ...
    expect(toIdentifiers(scanConvexAuthorization(root).unauthorized)).toEqual([
      "spoofed:commentOnly",
    ]);
  });

  it("does not treat an uncalled nested function as authorization", () => {
    const root = fixture({
      "nested.ts": `
        export const write = mutation({
          args: {},
          handler: async (ctx) => {
            async function unused() { await requireWorkspaceAccess(ctx, "t", "p", {}); }
            await ctx.db.insert("tasks", {});
          },
        });
      `,
    });
    expect(toIdentifiers(scanConvexAuthorization(root).unauthorized)).toEqual(["nested:write"]);
  });
  ...
  it("recognizes the audited Factory action authorization boundary", () => {
    const root = fixture({
      "factory/actions.ts": `
        export const improve = mutation({
          args: {},
          handler: async (ctx, args) => {
            await requireFactoryActionWithAudit(ctx, {
              projectId: args.projectId,
              permission: "factory.improve",
              operation: "factory.improve",
            });
          },
        });
      `,

...
const evidenceDirectory = path.join(repoRoot, "docs/testing/evidence/remote-sandbox-blocker-remediation-v1");
const registryProvenancePath = path.join(evidenceDirectory, "registry-image-provenance.json");
const resultsPath = path.join(evidenceDirectory, "live-3-workload-gate.json");
...
if (process.argv.slice(2).some((argument) => argument !== "--preflight")) {
  throw new Error("Usage: remote-sandbox-hardening-live-qualification.mts [--preflight]");
}

async function main() {
  const provenance = JSON.parse(await readFile(registryProvenancePath, "utf8"));
  ...
  if (process.env.MC_REMOTE_SANDBOX_HARDENING_LIVE !== "1" || process.env.MISSION_CONTROL_SANDBOX_LIVE !== "1") {
  ...
  const dataset: any = {
    ...
    baseline,
    constraints: {
      exactWorkloads: ["bug-fix", "security-policy", "data-migration"],
  ...
  try {
    ...
    dataset.aggregateNegativeMatrix = aggregateNegativeMatrix(dataset.executions);
    ...
    const allPassed = dataset.executions.length === 3
...
async function assertPreflight(provenance: any) {
  ...
  if (!/^ghcr\.io\/[^@]+@sha256:[a-f0-9]{64}$/.test(provenance.image)
...
async function runExecution(scheduled: (typeof schedule)[number], profile: any, qualificationRunId: string) {
  ...
  const negativeMatrix = securityNegativeMatrix(securityProof, cleanup, inventoryBefore, resourceName);
...
function executionPrompt(workload: any) {
  return [
    ...
    "When status is COMPLETED, every listed acceptance criterion ID must appear exactly once in completedAcceptanceCriterionIds, and incompleteAcceptanceCriterionIds and unknownAcceptanceCriterionIds must both be empty.",
...
function remoteProfile(checkedAt: number, provenance: any) {
  return {
    ...
    readiness: {
      ...
      liveCertified: true,
      evidenceReference: "docs/testing/evidence/remote-sandbox-blocker-remediation-v1/live-3-workload-gate.json",
    },
    security: {
...
function securityNegativeMatrix(proof: any, cleanup: any, inventoryBefore: any, resourceName: string | null) {
  ...
  const checks = {
    ...
    staleCredentialRejected: cleanup.staleCredentialRejected === true,
    providerAdminAuthorityAbsent: true,
    githubAuthorityAbsent: true,
...
function aggregateNegativeMatrix(executions: any[]) {
  const resourceNames = executions.map((execution) => execution.resourceName).filter(Boolean);
  const checks = {
    threeWorkloadsExecuted: executions.length === 3,
    ...
    uniqueAttemptResources: resourceNames.length === 3 && new Set(resourceNames).size === 3,
...
class RevocationProbingCredentialBroker implements SandboxCredentialBroker {
  ...
  readonly staleChecks: Array<{ grantKey: string; statuses: number[]; rejected: boolean; checkedAt: string }> = [];
  ...
  async revoke(grant: SandboxCredentialReference): Promise<SandboxCredentialRevocationReceipt> {
    const secret = this.secrets.get(grant.grantKey);
    const receipt = await this.inner.revoke(grant);
    if (!secret) throw new Error("Stale-credential probe lost its in-memory one-time secret.");
    ...
    for (let check = 0; check < 6; check += 1) {
      ...
      if (check < 5) await new Promise((resolve) => setTimeout(resolve, 750));
    }
    this.staleChecks.push({ grantKey: grant.grantKey, statuses, rejected, checkedAt: new Date().toISOString() });
    if (!rejected) throw new Error(`Revoked OpenRouter Attempt key remained usable or could not be proven rejected (statuses ${statuses.join(",")}).`);
    return receipt;
  }

...
} from "./lib/production-factory-pilot-v3.mjs";

const execFileAsync = promisify(execFile);
const repoRoot = path.resolve(path.dirname(new URL(import.meta.url).pathname), "..");
const evidenceDirectory = path.join(repoRoot, "docs/testing/evidence/production-factory-pilot-v3");
const runResultsPath = path.join(evidenceDirectory, "run-results.json");
const modelCatalogPath = path.join(repoRoot, "scripts/fixtures/remote-gpt-5.1-codex-mini-model-catalog.json");
const supportedModelCatalogRemotePath = "/var/lib/mission-control/attempt/model-catalog.json";
...
async function main() {
  ...
  let dataset: any = {
    ...
    priorEvidenceIntegrity: {
      unchanged: true,
      method: "Aggregate SHA-256 over sorted per-file SHA-256 records; verified again after V3 execution.",
      packets: {
        systemQualificationV1: { files: 14, aggregateSha256: "57200a1630cd3bdd2d0d7018d4215e0e80ac9fa06889645eb37131e6b2041702" },
        systemQualificationV2: { files: 16, aggregateSha256: "3a5c85831e87940b30e0cb507d10f51e51e440746fc5c0e471a5804e5ff18949" },
        productionFactoryPilotV1: { files: 27, aggregateSha256: "bb8e72d7a5d70100705d977a63efd19aa57c418d05b4a049a89a5f739240ee03" },
        productionFactoryPilotV2: { files: 19, aggregateSha256: "060fc50538b813ee3f3a94d97e46677fcbef4135a523e02eab6017a181541694" },
        remoteCodexStructuredOutputV1: { files: 14, aggregateSha256: "ac4e657649cc10e00cad5f99274d83c212eba306549c46ca9f45b1b5e3cddc93" },
        remoteRuntimeReliabilityV1: { files: 16, aggregateSha256: "7f33c2b46329e96d13d73395be0604ca448411ac2dbd82a6db185d36308fafc8" }
      }
    },
    ...
    v2ComparisonBaseline: {
      source: "Pilot V2 immutable BLOCKED evidence at a9d3df835449f2872e98c604c5cdc83dce7de136",
      governedExecutions: 15,
      eventualSuccesses: 14,
      localSuccesses: 12,
      localSamples: 12,
      remoteSuccesses: 2,
      remoteSamples: 3,
      attempts: 15,
      failedAttempts: 1,
      remoteStructuredOutputFailures: 0,
      retries: 0,
      contextMisses: 0,
      verificationFailures: 1,
      humanReviewCorrections: 0,
      firstPassStructuredResultRate: 1,
      firstPassVerificationRate: 0.9333,
      finalVmCount: 0,
    },
    remoteRuntimeQualification: {
      decision: "REMOTE RUNTIME RELIABILITY QUALIFIED",
      migration: { successes: 5, samples: 5, runtimeMs: 420_000, executorTimeoutMs: 390_000 },
      securityConfigurationD: { successes: 3, samples: 3 },
      freshRemoteGate: { successes: 3, samples: 3 },
      finalVmCount: 0,
    },
    failureInjections: [],
  ...
  if (options.finalizeExisting) {
    ...
    if (dataset.schemaVersion !== PILOT_SCHEMA || dataset.baseline?.sha !== expectedBaseline || dataset.executions?.length !== 15) {
      throw new Error("Existing pilot evidence does not match the complete frozen V3 pilot contract.");
    }
  ...
  dataset.decision = errors.length === 0
    ? "HUMAN-GOVERNED PRODUCTION PILOT READY"
    : dataset.executions.length >= 15
      && dataset.executions.every((execution: any) => execution.eventualSuccess)
      && dataset.remoteSandboxInventory?.vmCount === 0
        ? "QUALIFIED WITH OPERATIONAL LIMITATIONS"
        : "BLOCKED";
  await persist(dataset);
  await writeDerivedEvidence(dataset);
  process.stdout.write(`\nPilot result: ${dataset.decision}\nExecutions: ${dataset.executions.length}\n`);
  if (dataset.decision !== "HUMAN-GOVERNED PRODUCTION PILOT READY") process.exitCode = 1;
}
...
function normalizeExecutionTelemetry(execution: any) {
  ...
  return attachReviewTraversalEvidence(normalized);
}

function attachReviewTraversalEvidence(execution: any) {
  const review = execution.review;
  if (!review?.criterionTrace?.length) return execution;
  const latestAttempt = execution.attempts?.at(-1);
  const verification = latestAttempt?.verification ?? execution.verification;
  const envelopes = execution.evidence?.envelopes ?? [];
  return {
    ...execution,
    review: {
      ...review,
      criterionTrace: review.criterionTrace.map((trace: any) => {
        const criterionId = trace.criterion?.id;
        const criterionEvidence = envelopes.filter((envelope: any) => envelope.criterionId === criterionId);
        const evidenceIds = criterionEvidence.map((envelope: any) => envelope.id);
        return {
          ...trace,
          verificationChecks: [{
            id: `check-${execution.executionId}-${String(criterionId).toLowerCase()}`,
            name: verification?.command ?? trace.method,
            verifierId: verification?.verifierId ?? null,
            status: verification?.verdict === "VERIFIED" ? "PASSED" : "FAILED",
            evidenceIds,
          }],
          evidence: criterionEvidence.map((envelope: any) => ({
            id: envelope.id,
            verificationRunId: verification?.attemptId ?? envelope.verificationAttemptId,
            ...
            artifactReferences: [{
              ...
              digest: verification?.outputSha256 ?? null,
            }],
            contentHash: verification?.outputSha256 ?? null,
            sourceRevision: envelope.sourceRevision,
            candidateRevision: envelope.candidateRevision,
            recordedAt: latestAttempt?.finishedAt ?? null,
          })),
        };
      }),
    },
...
async function runGovernedExecution(entry: ScheduleEntry, pilotRunId: string, previous?: ExecutionRecord): Promise<ExecutionRecord> {
  ...
  for (let recoveryAttempt = 1; recoveryAttempt <= 2; recoveryAttempt += 1) {
    ...
    const governedConfiguration = qualifiedWorkloadConfiguration(entry.workload.key);
    const attempt: any = {
      attemptId,
      attemptNumber,
      ...
      governedConfiguration: {
        ...governedConfiguration,
        digest: `sha256:${canonicalHash(governedConfiguration)}`,
      },
      status: "RUNNING",
  ...
  return {
    ...
    context: {
      sufficient: true,
      contextPackageId: lineage.contextPackageId,
      contextPackageDigest: lineage.contextPackageDigest,
      missingSources: [],
      qualifiedWorkloadConfiguration: lineage.qualifiedWorkloadConfiguration,
    },
    attempts,
...
async function runRemoteCodex(
  ...
  const governedConfiguration = qualifiedWorkloadConfiguration(entry.workload.key);
  const profile = remoteProfile(governedConfiguration.runtimeMs);
  ...
  const manifest = {
    ...
    harness: {
      ...
      timeoutMs: governedConfiguration.executorTimeoutMs,
      provider: "openai",
      model: remoteModel,
    },
    sandbox: {
      resourceName: factorySandboxResourceName({
        projectId: "production-factory-pilot-v3",
        workflowRunId: publicRunId,
  ...
  addConfigOverride(invocation.args, `model_catalog_json="${supportedModelCatalogRemotePath}"`);
  ...
  try {
    const result = await runtime.execute({
      ...
      supervisorSource: await supportedSupervisorSource(),
      executor: {
        ...invocation,
        command: "npx",
    ...
    return {
      ...
      remoteResult: {
        ...
        governedConfiguration: {
          ...
          digest: `sha256:${canonicalHash(governedConfiguration)}`,
          profileDigest,
          modelCatalogDigest: sha256(await readFile(modelCatalogPath, "utf8")),
        },
...
function buildGovernedLineage(entry: ScheduleEntry, startedAt: number) {
  ...
  const qualifiedConfiguration = qualifiedWorkloadConfiguration(entry.workload.key);
  const contextPackage = {
    schema: "factory-context-package/v1",
    contextPackageId: `context-${entry.executionId}`,
    workloadKey: entry.workload.key,
    sourceFiles: Object.keys(entry.workload.files).sort(),
    implementationFiles: Object.keys(entry.workload.files).filter((name) => !name.startsWith("tests/") && name !== "package.json").sort(),
    testFiles: Object.keys(entry.workload.files).filter((name) => name.startsWith("tests/")).sort(),
    requirements: entry.workload.requirements,
    acceptanceCriteria: entry.workload.acceptanceCriteria,
    authority: "ADVISORY",
    sufficient: true,
    missingSources: [],
    additionalAuthorizedContext: qualifiedConfiguration.additionalAuthorizedContext,
  };
  ...
  const blueprint = {
    ...
    implementationPolicy: {
      ...
      maxLinesChanged: 300,
      maxAttempts: 2,
      timeoutMinutes: entry.backend === "remote-sandbox"
        ? Math.ceil(qualifiedConfiguration.executorTimeoutMs / 60_000)
        : 3,
  ...
  const qualityContract = compileApprovedPlanQualityContract({
    ...
    businessContext: "Production Factory Pilot V3 disposable operational workload.",
    ...
    specLineage: {
      ...
      projectConstitutionRevisionId: "constitution-production-factory-pilot-v3",
  ...
  return {
    constitutionRevisionId: "constitution-production-factory-pilot-v3",
    ...
    contextPackageId: `context-${entry.executionId}`,
    contextPackage,
    contextPackageDigest: `sha256:${canonicalHash(contextPackage)}`,
    qualifiedWorkloadConfiguration: {
      ...qualifiedConfiguration,
      digest: `sha256:${canonicalHash(qualifiedConfiguration)}`,
    },
    timings: {
...
function productionPilotSpec(entry: ScheduleEntry) {
  ...
  return {
    ...
    sources: [{ id: "SOURCE-001", kind: "DOC", label: "Production Factory Pilot V3", location: "todos/055-ready-p1-production-factory-pilot-v3-final-readiness-gate.md" }],
    clarifications: [],
...
function buildRepresentativeReview(entry: ScheduleEntry, lineage: any, attempt: any, candidate: any, semanticGroups: any[]) {
  ...
  const evidenceEnvelopes = criteria.map((criterion: any) => ({
    _id: criterion.evidenceEnvelopeIds[0],
    verificationRunId: `verification-${entry.executionId}`,
    sourceAttemptId: attempt.attemptId,
    producer: { actorId: `verifier-${entry.executionId}`, independent: true },
    category: "TEST_RESULT",
    artifactIds: [],
    artifactReferences: [{ kind: "VERIFICATION_OUTPUT", digest: attempt.verification.outputSha256 }],
    contentHash: attempt.verification.outputSha256,
  ...
  const projection = buildReviewIntelligenceProjection({
    ...
    verificationRun: {
      _id: `verification-${entry.executionId}`,
      verificationSubject: { kind: "GIT_CANDIDATE" },
      checks: criteria.map((criterion: any) => ({
        checkId: `check-${entry.executionId}-${criterion.id.toLowerCase()}`,
        name: attempt.verification.command,
        verifierId: attempt.verification.verifierId,
        status: attempt.verification.verdict === "VERIFIED" ? "PASSED" : "FAILED",
        acceptanceCriterionIds: [criterion.id],
        evidenceIds: criterion.evidenceEnvelopeIds,
      })),
    },
    evidenceEnvelopes,
    criteria,
  ...
  return {
    ...
    reviewPackage: {
      ...
      rawDiffDigest: sha256(candidate.diff),
    },
...
function executionPrompt(entry: ScheduleEntry, attemptNumber: number) {
  const configuration = qualifiedWorkloadConfiguration(entry.workload.key);
  return buildPilotExecutionPrompt(
    entry.workload.prompt,
    entry.workload.acceptanceCriteria,
    attemptNumber,
    configuration.additionalAuthorizedContext,
  );
}

function addConfigOverride(args: string[], value: string) {
  const execIndex = args.indexOf("exec");
  if (execIndex < 0) throw new Error("Codex invocation is missing the exec subcommand.");
  args.splice(execIndex, 0, "-c", value);
}

async function supportedSupervisorSource() {
  const source = standaloneSandboxSupervisorSource();
  const configDeclaration = 'const config = JSON.parse(readFileSync(process.argv[2], "utf8"));';
  const catalog = await readFile(modelCatalogPath);
  const configured = source.replace(
    configDeclaration,
    `atomicWrite("${supportedModelCatalogRemotePath}", Buffer.from("${catalog.toString("base64")}", "base64"));\n${configDeclaration}`,
  );
  if (configured === source) throw new Error("Remote supervisor source no longer exposes the governed model-catalog injection point.");
  return configured;
}
...
function workerIdentity(entry: ScheduleEntry, pilotRunId: string) {
  return {
    ...
    sessionId: `session-production-pilot-v3-${pilotRunId}-${entry.backend}`,
    generation: 1,
    current: true,
    maxConcurrentRuns: 1,
  };
...
function remoteProfile(runtimeMs: number) {
  return {
    schema: "factory-sandbox-profile/v1",
    profileKey: "exe-production-pilot-v3-n1",
    version: 1,
    provider: "EXE_DEV",
    providerProfile: "individual-small",
    providerProfileVersion: "production-pilot-v3",
    ...
    runtime: { maxRuntimeMs: runtimeMs, resultPollIntervalMs: 500, resultRetentionMs: 86_400_000 },
...
function finalizeRoutingShadow(shadow: any, executions: any[]) {
  ...
  return {
    ...
    evidenceCoverage: {
      localVerifiedSamples: executions.filter((item) => item.backend === "persistent-worker" && item.verification?.verdict === "VERIFIED").length,
      remoteVerifiedSamples: executions.filter((item) => item.backend === "remote-sandbox" && item.verification?.verdict === "VERIFIED").length,
      minimumVerifiedSamplesPerTuple: 5,
    },
    enoughEvidenceForFutureBoundedExperiment: executions.filter((item) => item.backend === "persistent-worker" && item.verification?.verdict === "VERIFIED").length >= 5
      && executions.filter((item) => item.backend === "remote-sandbox" && item.verification?.verdict === "VERIFIED").length >= 5,
...
function buildFactoryLearning(executions: any[], failureInjections: any[]) {
  ...
  return {
    historicalComparison: {
      v1: { structuredOutputFailures: 2, remoteSuccesses: 1, remoteSamples: 3, contextMisses: 0, verificationFailures: 2, humanCorrections: 0 },
      v2: { structuredOutputFailures: 0, remoteSuccesses: 2, remoteSamples: 3, contextMisses: 0, verificationFailures: 1, humanCorrections: 0 },
      v3: {
        structuredOutputFailures: executions.filter((execution) => !execution.firstPassStructuredResultSuccess).length,
        remoteSuccesses: executions.filter((execution) => execution.backend === "remote-sandbox" && execution.eventualSuccess).length,
        remoteSamples: executions.filter((execution) => execution.backend === "remote-sandbox").length,
        contextMisses: executions.filter((execution) => execution.context?.sufficient === false).length,
        verificationFailures: executions.filter((execution) => !execution.eventualVerificationSuccess).length,
        humanCorrections: executions.filter((execution) => execution.review?.correctionRequired === true).length,
      },
    },
    ...
    strongestExperimentProposal: strongest ? {
      id: "experiment-proposal-production-pilot-v3",
      candidate: strongest,
      status: "PROPOSED",
      autoPromote: false,
      releasedWorkOrders: [],
...
function buildHumanInterventions(executions: any[], preflightFailures: any[] = []) {
  const requiredGovernance = executions.flatMap((execution) => [
    ...
    { executionId: execution.executionId, category: "REVIEW_PACKAGE_APPROVAL", count: 1, acceptanceAuthority: false },
...
function buildOperationalMetrics(executions: any[]) {
  ...
  const observed = (key: string) => ({
    ...
    p95Ms: percentile(values(key), 0.95),
...
async function writeDerivedEvidence(dataset: any) {
  ...
  await Promise.all([
    ...
    writeFile(path.join(evidenceDirectory, "v1-v2-v3-comparison.json"), `${JSON.stringify({
      v1: dataset.v1ComparisonBaseline,
      v2: dataset.v2ComparisonBaseline,
      v3: {
    ...
    writeFile(path.join(evidenceDirectory, "prior-evidence-integrity.json"), `${JSON.stringify(dataset.priorEvidenceIntegrity, null, 2)}\n`),
